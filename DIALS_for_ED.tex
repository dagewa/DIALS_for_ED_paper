...
\emph{dataset 1} is shown by Figure~\ref{fig:spotfinding}.
...
With disregard of lens defects, discussed further in Section~\ref{sec:distortion},
the processing software can ignore these
distinctions and model the experiment with an effective detector. 
However,inaccuracy in the recorded effective distance may be difficult to correct by
...
with reciprocal metrical matrix elements as parameters \cite{Waterman2016}. However, for
...
stop and without overloading the detector. When images are not corrected,
...
\subsection{Structure solution and refinement}

The structure was determined as described \cite{Clabbers2017}, with the additional
step of refining the lattice in REFMAC5 \cite{Murshudov2011}. Lattice refinement
allows refining the unit cell by a single scaling factor independent of the sample
to detetor distance, thus removing the ambigiuty between detector distance and lattice
parameters. The newly found unit cell was then used for structure refinement and validation
\MC{add here a table with the stats and new cell?}. Electron scattering factors were used
for molecular replacement \cite{McCoy2007}, refinement \cite{Murshudov2011}, and validation
\cite{Joosten2014}.
