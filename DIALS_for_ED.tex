...
If the correct gain is known it is usual for this to be set by the format class
used to import images. Otherwise, a suitable value should be passed to
...
effects of which can be explored interactively using \dialsimageviewer. For
the 7 example datasets we typically found that it was necessary to increase
the sensitivity of spot-finding, and then reduce additional noise by using
a global threshold. Appropriate spot-finding settings were determined manually
for each dataset separately. The effect of these settings for dataset 1 is shown
by Figure~\ref{fig:spotfinding}.
...
\subsection{Joint refinement of the unit cell and instrument parameters \label{sec:refinement}}

\DW{split into two parts, static refinement and scan-varying, see issue no. 8}
...
done jointly with refinement of a beam and detector model. Like static
...
\begin{figure}
  ...
  \caption{
    ...
    same inset image after spot-finding settings were adjusted for this dataset.
    In this case, that amounted to setting \texttt{gain=0.833},
    \texttt{sigma\_strong=1.0} and \texttt{global\_threshold=1} as command-line
