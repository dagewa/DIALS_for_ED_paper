...
\cite{microed_method:2015,Clabbers2017}. The only very recent use of
...
invariant of linear scaling (see Section~\ref{sec:refinement}). The differences
between the diffraction geometry of typical X-ray and electron diffraction
cases in real and reciprocal space are summarised in Figure~\DW{mixed
reciprocal-real space Ewald sphere diagram with detector geometry - like
cartoons from the talk? If this can be done in a way that fits within 2
columns} \TG{the differences are small, but the consequences are larger - how
about a figure that compares the paramters cross-CC between X-rays and
electrons. That has also been part of your talk.} \DW{I think that is the
figure I mean to put in the Diagnostics section}
...
\subsection{Global refinement of the unit cell and instrument parameters \label{sec:refinement}}
...
with \dials, it is usual to first refine a ``static'' model for the whole data set,
in which parameters such as the crystal unit cell and orientation angles
are not allowed to vary across the scan. The global refinement of a data set improves the stability of the
...
refinement tasks with challenging electron diffraction data.
...
the radius of the sphere \DW{What does this sentence mean?}. Deviations from the correct unit cell parameters or
...
citation. Also - we do this for our example datasets, link to relevant section}.  While a change in
...
at each step of refinement. This was deemed necessary at the time to refine crystal
parameters using data from a single oscillation film. Within \dials, all
available data is used for a global refinement. This reduces correlations and
provides a better determination for parameters when the scan range is wide,
thus the default behaviour is to refine simultaneously the beam, crystal and
detector parameters, which works well for X--ray data. We have seen that
...
DIALS is expressed with metrical matrix elements as parameters
\cite{Waterman2016}. However, for ease of use, restraints are specified in terms of
the real space cell, as shown in Appendix~\ref{app:PHIL_example}.
Each crystal included in refinement can add up to six
restraint terms (for the triclinic case). Irrelevant restraints for cell
parameters that are already constrained by lattice symmetry are automatically excluded. Every
...
\DW{Remove this part?: An alternative method of restraining unit cells does not rely on an
...
information like refinement diagnostics, use of restraint, elliptical distortion,
beam drift, unit cell, average unit cell from scan-varying refinement are all
given?}

\subsection{Scan-varying refinement of crystal and beam parameters \label{sec:sv-refinement}}
...
cell and orientation parameters during data collection. These parameters were allowed to vary in a
smooth manner by evenly distributing sample points across the scan and
interpolating values at any one position using a Gaussian smoother. The beam
and detector parameters could be jointly refined to global, static values
alongside the scan-varying crystal. The analysis of electron diffraction
images raises a new issue in that instrument
stability during the course of data collection cannot be simply assumed, as it
is for MX data. In some cases, there is significant drift of the beam centre
during data collection. \DW{comments on the cause of these drifts, other
ways to handle it - cf. RED autocorrelation and writing out corrected images.
Also mentioned in a Hattne reference?}. Beam drift in electron diffraction
experiments, at least those collected by a continuous rotation protocol,
appears to occur gradually. Therefore it appears reasonable to assume that a
smoothly-varying model for the beam direction vector would suffice to represent
this effect. For that reason, we extended the scan-varying refinement
methodology from the crystal parameters to optionally apply also to the beam
...
\subsection{Diagnostics for problematic diffraction geometry refinement \label{sec:diag}}

\subsubsection{Simulating data for comparison}
\DW{Perhaps move description of simulation to supplementary material}
...
\subsubsection{Refinement diagnostics}
...
\DW{Show condition numbers for refinements of the real example data. Perhaps
compare refinement when detector and unit cell are refined simultaneously to
refinement when detector distance is fixed. Condition number and corrgrams
should improve}

\section{Multiple crystal restrained refinement \label{sec:group_cell_restraints}}
\DW{Probably remove this section}
