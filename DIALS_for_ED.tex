...
diffraction geometry. The short wavelength of an electron beam (0.0251 \AA\ for
200 keV electrons compared to 1.0332 \AA\ for 12 keV X-rays) implies a
correspondingly large Ewald sphere, with a small $2\theta$ scattering angle even
for the highest resolution reflections.  At an X-ray beamline the sample to
detector distance is easily calibrated with reliable rest crystals. With
electron diffraction, lens distortions increase the uncertainty, and radiation
hard calibration crystals suitable for unit cell parameters suitable for
proteins are difficult to find. For an X-ray experiment, the sample to detector
distance is limited by the desire to record the high angle reflections around
the base of the diffraction cone simultaneously. Large detectors are
advantageous for X-ray crystallography because they allow this distance limit to
be increased to both reduce diffuse X-ray background and to improve spatial
...
disregard of lens defects \cite{lenscorr_2dx:2006,Clabbers2017}, the processing
software can ignore this distinction and model the experiment with an effective
detector. However, inaccuracy in the recorded effective distance may be
difficult to correct by the usual process of diffraction geometry refinement due
to the high correlation between unit cell parameters and the detector distance
when $2\theta_{\text{max}}$ is small, when the Ewald sphere construction becomes
...
(see Figure \DW{see below}). Programs like AD3DT \cite{adt:2007} cannot
determine the direction of rotation from the program parameters and ignore it
(priv. comm. Ute Kolb). The ``classical'' rotation method enables the
uncertainties together with parameter refinement. \TG{I got the impression,
that the fact, that dials or xds give a clear clue about the rotation direction
could be quite a ``big thing'' in the electron diffraction community, but maybe
I exaggerate.} If indexing is successful, it is likely to work either way. For
any case where there is ambiguity, the inverse direction should also be tested
and results compared. The correct solution will have a lower RMSD for the
angular residual between the predicted and observed positions of the
reflections.

\TG{in my experience, the 'std dev. of spindle position is
significantly worse for the wrong direction. And the rotation axis is only
instrument dependent, i.e. one can calibrate the rotation with a high
resolution data set from a radiation hard sample.}
\DW{Original reply here
changed: I misunderstood the 'std dev. of spindle position' from XDS: now I
think this does not refer to refinement of the orientation of the rotation axis
by XDS, but is the equivalent of DIALS' \texttt{RMSD\_Z}. In that case, the statement about
``uncertainties together with parameter refinement'' is true, but irrelevant,
is it not? Assuming the rotation axis oriention is not refined and we are comparing
RMSDs between the two choices of direction, then for DIALS immediately after indexing
for the \texttt{288/sq\_05\_01} dataset gives correct axis: \texttt{RMSD\_Z}=0.58 images
inverted axis: \texttt{RMSD\_Z}=1.5 images. So, indeed the difference is significant. In
other cases the difference may be less dramatic, but the correct direction is
favoured unless there are other significant errors present confusing matters
(data I have from eBIC gives such poor solutions that I hesitate to claim
that lower \texttt{RMSD\_Z} for one solution definitely identifies the correct one)}

Most pernicious
is uncertainty in the rotation range per image. At an X-ray beamline the rotary
position and velocity of a goniometer is well-controlled and linked to the
detector and X-ray shutter (if used) so that accurate values can be written to
image headers during data collection. The rotation range per image is assumed to
be a constant. Data processing software does not usually provide the means to
automatically determine or refine this value, except indirectly via a crystal
orientation ``misset'' angle around the rotation axis. With DIALS, successful
indexing of an entire sweep is a prerequisite for refinement. If the rotation
range per image is uncertain, or worse variable, then the mapping of spot
positions from their positions on the detector and apparent rotation angle to
reciprocal space may not be sufficiently good for indexing to succeed. It
may be possible to find an indexing solution by trialling different values for
the image angular width, but in the face of other possibly gross errors the
results of a trial and error search can be difficult to interpret. It is
therefore advised that proper calibration of the sample stage is done ahead of
data collection \DW{FIXME how?} \TG{by a strip of reflecting facets around the
stage outside the microsope and a laser - or maybe even inside. Given planar
facets, should give fine step flickering of the reflected laser light. OMNY (PSI
isntrument at SLS) is the most sophisticated such device that I am aware of}.
Small, smooth deviations from the expected rotation angle can then be modelled
as part of the scan-varying refinement of the crystal.
...
often large variation in oscillation width $\Delta \Phi$.  \TG{This part about
indexing from 3D information seems misleading: already with mosflm, people
would at least collect two images 90$^\circ$ apart to get 3D info, and with
XDS, it has always been the default to use the entire sweep for spot finding
and thus indexing} The \dialsindex program offers three different methods for
determining the unit cell basis vectors. The default is based on the
three-dimensional FFT, but alternatively a method based on one-dimensional FFT
similar to the programs DPS \cite{Steller1997} and \mosflm \cite{leslie2007}
can be used. When the cell parameters are known, a simplification of the
Fourier transform-based methods can be used that is particularly successful for
very narrow wedges of data \cite{Gildea2014}. \DW{I made some changes here,
because dials.index does not have a method like IDXREF}
...
parameters can be refined as recently implemented in Refmac5. \TG{this works
really well and maybe that is another publication for you, me, and Oleg/Garib??}
While a change in the detector parameters can only result in a projective
transformation of the positions of the recorded spots on any image, changes in
the cell parameters will act to move the reciprocal lattice points towards or
away from the surface of the Ewald sphere, altering not only the predicted
positions of the spots on the detector, but also rotation angle and therefore
the predicted image number on which these spots are expected to appear. When the
Ewald sphere is essentially flat, this distinguishing factor is much reduced and
it is more difficult to separate the effects of the detector and unit cell
parameters.
...
\TG{If you don't refer to these equations, they can be omitted}
\DW{I'm planning to refer back later when discussing similarity restraints}
...
model, which had an orthorhombic unit cell with dimensions $a=34.17$, $b=73.23$
and $c=110.14$. To regularise the beam and detector models, we forced the beam
...
changing the wavelength from 0.0251~\AA\ to 1.0332~\AA\ and the detector model such
...
changed between them, and so on.} Electron crystallography takes place at
nanometre scale. This poses very high requirements on instrument precision,
which may just about be reached with dedicated diffraction
instruments\TG{reference to DESY/PETRA: 100nm sphere of confusion; and
SmarGON}, and electron microscopes are not dedicated for diffraction.
Generally, it is safer to assume that truly reproducible experimental geometry
cannot be achieved between runs, and models cannot be shared. Even so, it is
still possible to benefit from joint refinement by providing other links across
...
%non-isomorphism.
\TG{Frankly, I am not impressed with
BLEND, specifically because it relies on unit cell parameters. Kay's approach
based on CC1/2 and cross-CC seems much more plausible. I think there is a
reference now} \DW{Reference for Kay's xds\_nonisomorphism}
Electron diffraction experiments yield very large uncertainties for the unit
cell parameters.
By jointly refining multiple data sets with a restraint that
ties the individual cell parameters to their mean values, the overall variation
in the cell parameter values will be reduced. The strength of each restraint
can be adjusted by setting $\sigma$ values, as described before in
