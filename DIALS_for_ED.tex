...
Nevertheless, the DIALS software package can, with specific adaptations, deal with 
continuous rotation electron diffraction data. Common pathologies often encountered 
in electron diffraction make the data integration more challenging. Uncertainties 
can  arise from instrumentation, e.g. influencing the rotation range per image, or 
lead to distorted diffraction patterns from lens errors.  Radiation damage, beam drift, uncertainty of 
rotation axis direction, incomplete wedges of data, and strong correlation between 
lattice parameters and detector distance are additional factors that complicate data 
integration. Here we show recent features of DIALS for electron diffraction, allowing 
refinement with a varying beam model, applying corrections for distorted diffraction 
images, and enabling joint refinement of multiple incomplete wedges of rotation data. 
These novel features, combined with the existing tools in DIALS, make data integration
and refinement feasible for electron crystallography, even in highly pathological cases.
...
volume, electron diffraction of organic and macrmolecular \MC{macro} compounds was long
...
scattering effects and lens distortions, lead to much worse \MC{repalce 'lead to much 
...
\cite{microed_method:2015}. The very recent use of integration
