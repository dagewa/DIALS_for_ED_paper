...
\MC{upon re-reading, think would be worth to add this paragraph}
...
beam direction vector would suffice to represent this effect.
...
$\mu_1$ and $\mu_12$ in the nomenclature of \citeasnoun{Waterman2016}.
...
Besides errorrs occuring from instrumentation, there are additional issues 
specific to electron crystallography  that need to be adressed. In ED the 
measured kinematic signal is obscured by inelastic, dynamic, and multiple 
mixed scattering events. Inelastic and diffuse mixed scattering increase
the background signal, and focused mixed scattering leads to a broadening
of the observed Bragg peaks. Zero-loss energy fitlering is a instrumtenal
solution to this problem and can filter out most of the inelastically 
scattered electrons, reducing the diffuse background and sharpening Bragg
peaks \cite{Yonekura2002}.

\MC{TBA: comment on this line from the abstract: Dynamic
scattering, absorption, radiation damage and incomplete wedges of data are
additional factors that complicate data processing.}

It is hoped that the presentation of these results will encourage other users to
investigate the use of \dials for processing of electron diffraction data. The
toolkit design philosophy of the software, including an extensible image format
...
\begin{tabular}{ll}
  ...
  Resolution ($\SI{}{\angstrom}$) & 56.78--2.10 (2.16--2.10) \\
  $R_\textrm{merge}$ & 0.488 (0.451) \\
  $R_\textrm{meas}$ & 0.534 (0.562) \\
  $R_\textrm{pim}$ & 0.208 (0.330) \\
  Observations & 50999 (1504) \\
  Completeness (\%) & 67.6 (51.5) \\
  $\langle I/\sigma(I) \rangle$ & 2.6 (1.4) \\
  $CC_{{1}/{2}}$ (\%) & 90.6 (62.7) \\

  \hline
  Refinement \\
  \hline
  Unit cell dimensions\fnmark[1] \\
  $a$, $b$, $c$ ($\SI{}{\angstrom}$) & 104.36, 67.49, 31.81 \\
  $\alpha$, $\beta$, $\gamma$ ($^\circ$) & 90 90 90 \\
  Resolution ($\SI{}{\angstrom}$) & 56.67--2.10 \\
  Reflections & 9295 \\
  $R1$\fnmark[2] (\%) & 25.3 \\
  $R_\textrm{complete}$\fnmark[2] (\%) & 29.9 \\
  $\langle B \rangle$ ($\SI{}{\angstrom}^2$) & 18.3 \\
  $RmsZ$ bond lengths & 0.50 \\
  $RmsZ$ bond angles & 0.72 \\
  Ramachandran \\
  favoured, allowed, outliers (\%) & 95.7 4.3 0.0\\

\end{tabular} \fntext[1]{Unit cell dimensions after lattice parameter
refinement in REFMAC5 by a single scaling factor \cite{Murshudov2011}}
\fntext[2]{$R_\textrm{complete}$ is a more robust validation method, especially
in cases where the data completeness is limited, making use of all reflections
\cite{Brunger1997}, and thus $R_\textrm{work}$ is equivalent to $R1$.
...
\DW{This should move to supplementary material rather than an appendix}
