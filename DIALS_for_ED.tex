...
uncertainties together with parameter refinement. \TG{I got the impression, that
the fact, that dials or xds give a clear clue about the rotation direction could
be quite a ``big thing'' in the electron diffraction community, but maybe I
exaggerate.} If indexing is successful, it is likely to work
either way, with only slightly worse RMSDs between the predicted and observed
positions for the incorrect choice as guidance. \TG{in my experience, the 'std
dev. of spindle position is significantly worse for the wrong direction. And the
rotation axis is only instrument dependent, i.e. one can calibrate the rotation
with a high resolution data set from a radiation hard sample.}
\DW{Interesting - DIALS does not yet allow refinement of the spindle axis
orientation. The canonical imgCIF coordinate frame is assumed in which the
rotation axis is along X, and the beam, crystal and detector orientations
refine around that. I should check the standard deviations of model parameters
for these other models. Otherwise, I plan to add refinement of the rotation
axis direction rather soon anyway.}
Most pernicious
