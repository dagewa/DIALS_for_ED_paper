...
Within \dials, the image distortion due to lens effects is handled via the use
of distortion maps which encode the pixel offset across the detector for both
the fast and slow directions, The distortion maps are applied during the
conversion between detector pixel coordinates and virtual detector millimetre
coordinates. During the transformation from from millimetre coordinates to pixel
coordinates, the uncorrected pixel coordinate is first calculated and the
correction is applied to obtain the distortion corrected pixel coordinate.
Likewise, during the transformation from pixel coordiantes to millimetre
coordinates, the reverse correction is first applied and the millimetre
coordinate is calculated from the reverse corrected pixel coordinate.
...
\caption{
  ...
  \DW{Information to add:
  condition number,
  use of restraint,
  elliptical distortion,
  beam centre,
  unit cell,
  average unit cell from scan-varying refinement,
  final RMSDs}
}
