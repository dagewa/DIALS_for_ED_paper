...
el = ExperimentListFactory.from_json_file('static.json', check_format=False)
exp = el[0]
beam = exp.beam
...
diagonal = (matrix.col(detector[0].get_pixel_lab_coord((0,0))) -
    matrix.col(detector[3].get_pixel_lab_coord(detector[3].get_image_size())))
...
detector2 = DetectorFactory.simple(sensor='PAD',
                                   distance=distance,
...
exp.scan = ScanFactory.make_scan(image_range, exposure_times,
...
detector3 = DetectorFactory.simple(sensor='PAD',
                                   distance=distance,
                                   beam_centre=beam_centre,
                                   fast_direction='+x',
                                   slow_direction='-y',
